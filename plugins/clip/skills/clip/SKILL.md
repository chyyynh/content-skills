...
name: clip
description: >
  Clips a YouTube video locally using yt-dlp and ffmpeg. Produces two output tiers:
  16:9 clips (main.mp4) and 9:16 short videos (final.mp4 = hook intro + main clip + optional CTA).
  Supports auto-highlight detection, translation, and CapCut-style karaoke subtitle burning.
  For 9:16 format, hook intro with AI cover and voiceover is generated by default.
  Env vars: GROQ_API_KEY (Whisper fallback), ELEVENLABS_API_KEY (hook voiceover), OPENROUTER_API_KEY (AI cover).
...
- **16:9 clip** → steps 1–8 → `main.mp4` (final output)
- **9:16 short video** → steps 1–8 → `main.mp4` (intermediate) → step 9 → `final.mp4` (final output)

For 9:16 format, `main.mp4` is NOT the final deliverable. Step 9 adds a hook intro (AI cover + voiceover) and optional CTA outro, then concatenates everything into `final.mp4`.
...
### 0. Prerequisites and user preferences

**Tools** — verify all are available:

```bash
command -v yt-dlp && command -v ffmpeg && command -v python3
```

**API keys** — detect availability (do not ask the user to provide them):

```bash
echo "GROQ: ${GROQ_API_KEY:+set}" "ELEVENLABS: ${ELEVENLABS_API_KEY:+set}" "OPENROUTER: ${OPENROUTER_API_KEY:+set}"
```

**Gather preferences upfront** — ask in one prompt while subtitles download:

```
1. 格式：16:9 橫版 或 9:16 短影片？
2. 字幕樣式：雙語卡拉OK / 純翻譯 / 純原文卡拉OK / 不加字幕
3.（僅 9:16）Hook 開場：要（預設）/ 不要
```

Use the answers to skip inapplicable steps: skip step 5 if subtitle style is "原文 only" or "no subs"; skip step 9 if format is 16:9 or user opts out of hook intro.
...
Based on the subtitle style chosen in step 0, generate the appropriate ASS file:
...
**Default for 9:16 format.** Skip only if the user explicitly opted out in step 0.

Structure: `intro.mp4` (cover + hook voiceover, 3–8s) → `main.mp4` → `[outro.mp4]` → `final.mp4`

#### a. Plan hook script and cover

Read [references/hook-formulas.md](references/hook-formulas.md), analyze the transcript, pick a formula, and write: hook text (2–4 sentences), cover title (≤10 chars) + subtitle. Present to user for approval, then generate cover image with `$IMG_SCRIPT` or ffmpeg drawtext fallback. See [references/short-video-intro.md](references/short-video-intro.md) steps a–b for detailed workflow.

#### b. Generate voiceover

```bash
# Requires ELEVENLABS_API_KEY. If not set, use anullsrc silent audio in step c.
curl -s -X POST "https://api.elevenlabs.io/v1/text-to-speech/<VOICE_ID>?output_format=mp3_44100_128" \
  -H "xi-api-key: $ELEVENLABS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"text": "<HOOK_TEXT>", "model_id": "eleven_multilingual_v2",
...
**Critical**: intro must match main clip's fps and sample rate.
...
#### d. Concat final video

**Must re-encode** — do NOT use `-c copy` (different AAC profiles cause audio drops).
...
See [references/short-video-intro.md](references/short-video-intro.md) for: voice listing, CTA outro creation, fallback cover without image plugin, and silent audio variant.
